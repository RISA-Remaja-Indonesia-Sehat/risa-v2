...
const getMotivationalMessage = (score, isMemoryGame) => {
  ...
  if (score >= 100) {
    grade = "S Rank";
    message =
      "LEVEL MASTER! Kamu mendominasi permainan ini. Pengetahuan kamu LUAR BIASA! 🏆";
    ...
    grade = "A+";
    message =
      "FANTASTIS! Sedikit lagi mencapai skor sempurna! Pertahankan fokus dan coba raih nilai sempurna!";
...
const postScoreToAPI = async (data, userId) => {
  ...
  const durationSeconds = Number.isFinite(data?.duration_seconds)
    ? data.duration_seconds
    : toSeconds(data?.duration ?? data?.time ?? 0);
  const correctCount =
...
const QuizDetailCard = ({ answer, index }) => (
  ...
  >
    ...
    <p className="text-gray-700 font-medium mb-3 text-sm">
      {answer.statement || answer.question || "Teks Pertanyaan Tidak Ditemukan"}
    </p>
...
export default function ResultPage() {
  ...
  useEffect(() => {
    ...
    const isAiGameType =
      data &&
      (data.gameType === "DRAG_DROP" ||
        data.gameType === "mitosfakta" ||
        data.gameType === "MEMO_CARD" ||
    ...
    if (isAiGameType && !aiRequestSent) {
      ...
      const normalizedAnswers =
        ...
        data.gameType === "CROSSWORD"
          ? data.answers || []
          : (data.answers || []).map((p) => ({
              statement: `${p.term || "Istilah"} — ${
                p.definition || "Definisi"
              }`,
              userAnswer: "matched",
      ...
      const normalizedScore =
        data.gameType === "MEMO_CARD" || data.gameType === "memory"
      ...
      fetch("/api/ai-analysis", {
        ...
        .then((payload) => {
          ...
          setAiArticles(
            Array.isArray(payload?.recommendedArticles)
              ? payload.recommendedArticles
              : []
          );
        })
        .catch((err) => {
          ...
          setAiAnalysis(
            "RISA lagi sibuk sebentar. Ini ringkasan singkat: kamu sudah bagus, lanjut asah topik yang salah, ya!"
          );
          setAiArticles([]);
        })
  ...
  const handleShareResult = useCallback(async () => {
    ...
    const score =
      gameType === "MEMO_CARD" || gameType === "memory"
  ...
  const isMemoryGame =
    resultData.gameType === "memory" || resultData.gameType === "MEMO_CARD";
  const isAiGame =
    resultData.gameType === "DRAG_DROP" ||
    resultData.gameType === "mitosfakta" ||
    resultData.gameType === "MEMO_CARD" ||
    resultData.gameType === "memory" ||
    resultData.gameType === "CROSSWORD";
  ...
  const correctValue = isMemoryGame
    ? resultData.matchedCount ?? resultData.correct_answer ?? 0
    : resultData.correct ?? 0;
  const wrongValue = isMemoryGame
    ? resultData.total_moves ?? resultData.moves ?? 0
    : resultData.wrong ?? 0;
  const scoreValue = isMemoryGame
    ? resultData.points ?? resultData.score ?? 0
    : resultData.score ?? resultData.points ?? 0;
  const rawTime =
    resultData.time ?? resultData.duration ?? resultData.duration_seconds;
  const displayTime =
    typeof rawTime === "string" ? rawTime : `${Number(rawTime || 0)}s`;
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-pink-100 via-white to-gray-50">
      ...
      >
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-2xl md:text-3xl font-black text-pink-500 mb-2 drop-shadow-md">
            Kamu mendapat
          </h1>
          ...
          >
            ...
            <div className="w-48 h-48 md:w-64 md:h-64 mx-auto mb-8 bg-gradient-to-br from-pink-500 to-rose-600 rounded-full flex flex-col items-center justify-center text-white shadow-xl shadow-pink-400/50 transition-shadow duration-500">
              <span className="text-base font-medium opacity-80">
                {isMemoryGame ? "TOTAL POIN" : "TOTAL NILAI"}
              </span>
              <span
                id="score-number"
            ...
            <div
              ...
              className={`p-4 md:p-5 rounded-2xl shadow-lg mx-auto max-w-lg mb-3 opacity-0 transform translate-y-10 border-b-4 border-pink-200 ${badgeColor}`}
            >
              <div className="flex items-center justify-center gap-3">
                <GradeIcon className={`w-8 h-8 ${color}`} />
                <span
                  className={`text-2xl md:text-3xl font-extrabold ${color}`}
                >
                  {grade}
                </span>
                <GradeIcon className={`w-8 h-8 ${color}`} />
              ...
              <p
                className={`text-md md:text-xl font-bold mt-3 ${color} leading-snug`}
              >
                {message}
              </p>
            </div>
            ...
            <div
              id="action-buttons"
              className="mt-2 flex flex-col sm:flex-row justify-center gap-3 md:gap-4 opacity-0 transform translate-y-10"
            >
              <CustomButton
                id="share-btn-game"
                title="Bagikan Hasil"
                text="Bagikan Hasilmu"
                onClick={handleShareResult}
                className="text-base md:text-lg min-w-[200px] flex items-center justify-center gap-2 px-6 py-3"
                role="button"
              />
            </div>
          </div>
          ...
          <div className="text-left mt-12 md:mt-16 opacity-0" id="result-list">
            <h2 className="text-xl md:text-3xl font-black text-pink-600 mb-6 border-b-4 border-pink-300 inline-block pb-1">
              ...
              resultData.answers.length > 0 ? (
                ...
                <div className="col-span-1 md:col-span-2">
                  <div className="p-4 md:p-5 rounded-xl border border-pink-200 bg-pink-50/80 text-center text-pink-700 font-medium shadow-sm">
                    {isMemoryGame
                      ? "Belum ada pasangan tepat yang ditemukan pada permainan ini."
                      : "Belum ada detail jawaban untuk ditampilkan."}
                  </div>
                </div>
              )}
            </div>
          ...
          >
            {isAiGame ? (
                ...
                {aiArticles.length > 0 ? (
                  ...
                  !isAiLoading && (
                    <p className="text-center text-lg text-gray-600 mb-6">
                      Kerja bagus! RISA tidak memiliki rekomendasi tambahan saat
                      ini.
                    </p>
                  )
                )}
              ...
              <>
                <p className="text-center text-lg text-gray-600 mb-6">
                  Yuk dibaca! Artikel ini akan bantu tingkatkan pengetahuan dan
                  poin di game berikutnya. 🚀
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  <Link
                    href="/article/1"
